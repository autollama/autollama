...
// Legacy route removed for memory optimization
...
// Placeholder - search route moved to active section

// FIXED: Documents endpoint - now properly returns the 205 documents
app.get('/api/documents', async (req, res) => {
    console.log('📋 Documents endpoint called with query:', req.query);
    try {
        const { limit = 20, offset = 0 } = req.query;
        
        // Use getSmartContentMix to get the correct 205 documents
        const result = await db.getSmartContentMix();
        const allDocuments = result.records || [];
        ...
        console.log(`📋 Returning ${paginatedDocs.length} of ${allDocuments.length} total documents`);
...
// MOVED: Document chunks API moved to before service initialization (line ~4351)

// DOCUMENT CHUNKS ROUTE REMOVED - HANDLED BY NEW ROUTE SYSTEM
// Original route functionality moved to new route system
...
app.get('/api/chunks', async (req, res) => {
    try {
        ...
        console.log(`🧩 Chunks API called (limit: ${limit}, offset: ${offset})`);
        const startTime = Date.now();
        ...
        const responseTime = Date.now() - startTime;
        console.log(`✅ Chunks API completed in ${responseTime}ms, returned ${safeChunks.length} chunks`);
        
        res.json(safeChunks);
        
    } catch (error) {
        console.error('❌ Chunks API error:', error.message, error.stack);
        res.status(500).json({ 
...
// Legacy search routes - reactivated due to new route system issues
console.log('🔍 REGISTERING LEGACY SEARCH ROUTE at /api/search');
app.get('/api/search', async (req, res) => {
    console.log('🔍 LEGACY SEARCH ROUTE HIT!', req.query);
    try {
...
// Test search endpoint to verify database functionality
console.log('🔍 REGISTERING TEST SEARCH ROUTE at /api/test-search');
app.get('/api/test-search', async (req, res) => {
    console.log('🔍 TEST SEARCH ROUTE HIT!', req.query);
    try {
...
app.get('/api/system/status', async (req, res) => {
    ...
    try {
        ...
        // Get memory and connection stats
        ...
        res.json({
            ...
            connections: {
                database: {
                    ...
                    total: connectionStats.poolStats?.totalCount || 0,
                    leaks: connectionStats.connectionLeaks,
                    healthy: connectionStats.healthy
                }
            },
...
app.get('/api/settings', async (req, res) => {
    try {
        console.log('🔧 API Settings GET endpoint called');
...
app.post('/api/settings', async (req, res) => {
    try {
        console.log('🔧 API Settings POST endpoint called');
        ...
        console.log(`✅ Updated ${results.length} API settings`);
        ...
        console.error('❌ Error updating API settings:', error.message);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
app.get('/api/settings/:key', async (req, res) => {
    try {
        ...
        console.log(`🔧 API Settings GET single key: ${key}`);
        ...
        console.error(`❌ Error fetching API setting ${req.params.key}:`, error.message);
        res.status(500).json({
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
    ...
    async parseEPUB(buffer) {
        return new Promise((resolve, reject) => {
            ...
            tmp.file({ postfix: '.epub' }, (err, path, fd, cleanupCallback) => {
                ...
                console.log('📖 EPUB: Writing buffer to temp file for parsing');
                require('fs').writeFile(path, buffer, (writeErr) => {
                    ...
                    console.log('📖 EPUB: Starting EPUB parser');
                    // Parse EPUB
                    ...
                    epubParser.on('end', () => {
                        ...
                        console.log(`📖 EPUB: Found ${totalChapters} chapters to process`);
                        ...
                        const chapterTimeout = setTimeout(() => {
                            console.error('❌ EPUB chapter processing timeout');
                            clearTimeout(parseTimeout);
                            cleanupCallback();
                        ...
                        chapters.forEach((chapter, index) => {
                            epubParser.getChapter(chapter.id, (error, text) => {
                                if (!error && text) {
                                    ...
                                    content += chapterText + '\n\n';
                                    console.log(`📖 EPUB: Processed chapter ${index + 1}/${totalChapters} (${chapterText.length} chars)`);
                                } else if (error) {
                                    console.warn(`⚠️ EPUB: Failed to read chapter ${index + 1}:`, error.message);
                                }
                                ...
                                if (processed === totalChapters) {
                                    ...
                                    cleanupCallback();
                                    console.log(`✅ EPUB: Completed processing ${totalChapters} chapters (${content.length} total chars)`);
                                    resolve({
...
// Legacy file upload route removed - now using background queue service

// Legacy non-streaming file upload route removed - now using background queue service
...
app.post('/api/cleanup-sessions/advanced', async (req, res) => {
    try {
        console.log('🧹 Advanced manual cleanup requested');
        ...
        console.error('❌ Advanced manual cleanup failed:', error);
        res.status(500).json({
            success: false,
...
app.get('/api/upload-sessions/check-stuck', async (req, res) => {
    try {
        ...
        console.error('❌ Check stuck sessions error:', error);
        res.status(500).json({
            success: false,
...
app.post('/api/upload-sessions/cleanup-stuck', async (req, res) => {
    try {
        console.log('🧹 Frontend-requested session cleanup');
...
function createHeartbeatMonitoredCallback(originalCallback, uploadSession, filename) {
    ...
    const heartbeatInterval = setInterval(async () => {
        ...
        if (timeSinceLastHeartbeat > 90000) {
            console.warn(`⚠️ Heartbeat timeout detected for ${filename} (${timeSinceLastHeartbeat}ms since last update)`);
